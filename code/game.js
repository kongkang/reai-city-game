...
// 默认使用mapData中的地图数据
let currentMapData = window.mapData || [];
...
function isTileWalkable(tileX, tileY) {
    ...
    // 检查图块类型是否可通行 (GRASS 和 DOOR 可通行，WALL 不可通行)
...
function moveCharacter(tileX, tileY) {
    ...
    // 检查是否站在门上，并触发相应行为
    if (currentMapData && currentMapData[tileY] && currentMapData[tileY][tileX] === TILE_TYPES.DOOR) {
        console.log(`角色站在门口，可以进入建筑物`);
        // 这里可以添加进入建筑物的逻辑
    }
